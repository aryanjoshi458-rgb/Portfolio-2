...
const performLogin = () => {
    if (passwordInput.value === 'admin123') {
        ...
        btnLogin.innerHTML = `<span style="display: inline-block; width: 16px; height: 16px; border: 2px solid rgba(255,255,255,0.3); border-radius: 50%; border-top-color: #fff; animation: btn-loading-spin 0.8s linear infinite; margin-right: 10px; vertical-align: middle;"></span> Authenticating...`;

        setTimeout(() => {
            setAuth();
            
            // Remove early-auth style sheet to clear the "!important" overrides
            const earlyAuthStyle = document.getElementById('early-auth-style');
            if (earlyAuthStyle) earlyAuthStyle.remove();
        ...
        }, 1000); // 1-second delay for smooth authentication feeling
    } else {
...
function fetchStats() {
    fetch(`${API_BASE_URL}/api/stats`)
        ...
        .then(data => {
            ...
            const messagesEl = document.getElementById('stat-total-messages');
            if (visitorsEl) visitorsEl.textContent = data.unique_visitors || 0;
            if (messagesEl) messagesEl.textContent = data.total_messages || 0;
        })
        .catch(err => console.error("Failed to load dashboard stats:", err));
}

// Trigger initial stats load on script startup
fetchStats();
...
function loadSettings() {
    ...
    fetch(`${API_BASE_URL}/api/portfolio`)
        ...
        .catch(err => {
            console.error('API server load failed, falling back to localStorage:', err);
            storedFallback();
        });
}
        ...
        portfolioData.certs.forEach(c => {
            ...
            div.innerHTML = `
                        ...
                        <div class="project-item-actions">
                            ...
                            <button class="btn btn-secondary" onclick="deleteCert('${c.id}')" style="padding: 6px 12px; font-size: 0.85rem; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: transparent;">Delete</button>
                        </div>
                    `;
        ...
        portfolioData.education.forEach(e => {
            ...
            div.innerHTML = `
                        ...
                        <div class="project-item-actions">
                            ...
                            <button class="btn btn-secondary" onclick="deleteResumeItem('education', '${e.id}')" style="padding: 6px 12px; font-size: 0.85rem; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: transparent;">Delete</button>
                        </div>
                    `;
            container.appendChild(div);
        });
        ...
        portfolioData.experience.forEach(e => {
            ...
            div.innerHTML = `
                        ...
                        <div class="project-item-actions">
                            ...
                            <button class="btn btn-secondary" onclick="deleteResumeItem('experience', '${e.id}')" style="padding: 6px 12px; font-size: 0.85rem; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: transparent;">Delete</button>
                        </div>
                    `;
            container.appendChild(div);
        });
        ...
        portfolioData.testimonials.forEach(t => {
            ...
            div.innerHTML = `
                        ...
                        <div class="project-item-actions">
                            ...
                            <button class="btn btn-secondary" onclick="deleteTestimonial('${t.id}')" style="padding: 6px 12px; font-size: 0.85rem; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: transparent;">Delete</button>
                        </div>
                    `;
            container.appendChild(div);
        });
    ...
    portfolioData.projects.forEach(p => {
        ...
        div.innerHTML = `
                    ...
                    <div class="project-item-actions">
                        ...
                        <button class="btn btn-secondary" onclick="deleteProject('${p.id}')" style="padding: 6px 12px; font-size: 0.85rem; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: transparent;">Delete</button>
                    </div>
                `;
        container.appendChild(div);
    });
...
document.getElementById('btn-global-save').addEventListener('click', () => {
    ...
    fetch(`${API_BASE_URL}/api/portfolio`, {
        ...
        .catch(err => {
            console.error('Database save failed. Data saved to localStorage only:', err);
...
function checkMessagesBadge() {
    fetch(`${API_BASE_URL}/api/messages`)
        ...
        .catch(err => console.error('Could not check messages count:', err));
}
...
const skillModal = document.getElementById('skill-form-container');
...
function renderSkillsList() {
    ...
    // Safe fallback if skills array doesn't exist yet
    if (!portfolioData.skills) {
        portfolioData.skills = [
            // Frontend
            { id: "1", name: "HTML5 / Semantic UI", percent: 95, category: "frontend" },
            { id: "2", name: "CSS3 / Flexbox / Grid", percent: 90, category: "frontend" },
            { id: "3", name: "JavaScript (ES6+)", percent: 92, category: "frontend" },
            { id: "4", name: "React / Next.js", percent: 88, category: "frontend" },
            // Backend
            { id: "5", name: "Node.js / Express.js", percent: 85, category: "backend" },
            { id: "6", name: "Python / FastAPI", percent: 80, category: "backend" },
            // Database
            { id: "7", name: "PostgreSQL / SQLite", percent: 85, category: "database" },
            { id: "8", name: "MongoDB", percent: 82, category: "database" },
            // Tools
            { id: "9", name: "Docker / Containerization", percent: 75, category: "tools" },
            { id: "10", name: "Git / GitHub / CI-CD", percent: 90, category: "tools" },
            { id: "11", name: "AWS Cloud Services", percent: 78, category: "tools" },
            // AI & APIs
            { id: "12", name: "OpenAI / Gemini API Integration", percent: 85, category: "ai" }
        ];
    }
    ...
    portfolioData.skills.forEach(s => {
        ...
        if (nameLower.includes('html')) {
            ...
            deviconClass = 'devicon-google-plain colored';
        } else {
        ...
        item.innerHTML = `
                    <div class="project-item-info" style="display: flex; align-items: center; gap: 15px;">
                        <div style="display: flex; align-items: center; justify-content: center; width: 40px; height: 40px; font-size: 2rem; border-radius: 6px; background: rgba(255, 255, 255, 0.02); border: 1px solid var(--glass-border); padding: 5px;">
                        ...
                        <div>
                            <strong style="font-size: 1.1rem; color: var(--text-primary);">${s.name}</strong>
                            <div style="font-size: 0.85rem; color: var(--text-secondary); margin-top: 4px;">
                                Category: <span style="text-transform: capitalize; color: var(--accent-primary);">${s.category}</span> | Level: ${s.percent}%
                            </div>
                        </div>
                    </div>
                    <div class="project-item-actions">
                        <button class="btn btn-secondary" onclick="editSkill('${s.id}')" style="padding: 6px 12px; font-size: 0.85rem;">Edit</button>
                        <button class="btn btn-secondary" onclick="deleteSkill('${s.id}')" style="padding: 6px 12px; font-size: 0.85rem; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: transparent;">Delete</button>
                    </div>
                `;
...
window.editSkill = function (id) {
    const s = portfolioData.skills.find(x => x.id === id);
    if (!s) return;
    document.getElementById('skill-id').value = s.id;
    document.getElementById('skill-name-input').value = s.name;
    document.getElementById('skill-percent-input').value = s.percent;
...
window.deleteSkill = function (id) {
    showConfirm('Are you sure you want to delete this skill?', () => {
        portfolioData.skills = portfolioData.skills.filter(x => x.id !== id);
        renderSkillsList();
        document.getElementById('btn-global-save').click(); // Auto save settings
    });
};
...
if (btnAddSkill) {
    btnAddSkill.addEventListener('click', () => {
        ...
        document.getElementById('skill-percent-input').value = '90';
        document.getElementById('skill-category-input').value = 'frontend';
...
if (btnSaveSkill) {
    btnSaveSkill.addEventListener('click', () => {
        ...
        const percent = parseInt(document.getElementById('skill-percent-input').value) || 0;
        ...
        skillModal.style.display = 'none';
        renderSkillsList();
        document.getElementById('btn-global-save').click(); // Auto-save update
    });
}
...
/* Floating Scroll Helper widget control */
function initScrollHelper() {
    ...
    const btnMid = document.getElementById('btn-scroll-mid');
    const btnLast = document.getElementById('btn-scroll-last');
    
    if (!helper || !btnTop || !btnMid || !btnLast) return;

    let isScrollingTimeout;
    
    function checkVisibility() {
        // Only show if the page height is scrollable (longer than viewport)
        const isScrollable = document.documentElement.scrollHeight > window.innerHeight;
        
        if (isScrollable && window.scrollY > 150) {
            // Show with a smooth transition
            helper.style.opacity = '1';
            helper.style.visibility = 'visible';
            helper.style.transform = 'translateY(0) scale(1)';
            
            // Clear the timeout to keep it visible while scrolling
            window.clearTimeout(isScrollingTimeout);
            
            // Hide the helper after 2.5 seconds of scroll inactivity
            isScrollingTimeout = setTimeout(() => {
                helper.style.opacity = '0';
                helper.style.visibility = 'hidden';
                helper.style.transform = 'translateY(20px) scale(0.95)';
            }, 2500);
        } else {
    ...
    // Keep it visible if the user is hovering over it
    ...
    btnMid.addEventListener('click', () => {
        // Scroll to half of scrollable height
        const midPosition = (document.documentElement.scrollHeight - window.innerHeight) / 2;
        window.scrollTo({
            top: midPosition,
            behavior: 'smooth'
        });
...
// Restore active tab on load (default to overview dashboard)
const savedTab = sessionStorage.getItem('admin-active-tab') || 'overview';
switchTab(savedTab);
